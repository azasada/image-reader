...
def index():
    ...
    crop = 0
    if (request.values.get("crop")  == "on"):
        crop = 1

    spellcheck = 0
    if (request.values.get("spellcheck") == "on"):
        spellcheck = 1

    # print(crop, spellcheck)
    for image in images:
        image_data = image.read()

        filename = "tmp." + image.filename[image.filename.index(".")+1:]
        image.stream.seek(0)
        image.save(filename)
        if crop:
            cropping(filename)
